...
class FacadeMakeCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'make:facade {name : The name of the facade to be created}';
    ...
    {\n\t\t".'$this'."->app->singleton('$name', function () {\n\t\t\treturn new $name(new $model);\n\t\t});\n"],
            ...
            if (!File::exists(app_path('/Providers/EloquentServiceProvider.php'))) {
                if ($this->confirm('The service provider for eloquent repositories does not exist! Do you want to create it?')) {
                    $defaultProviderContent = File::get(package_path('Commands/stubs/EloquentServiceProvider.stub'));
                    $runtimeProviderContent = str_replace('Dummy', $model, $defaultProviderContent);
                    File::put(package_path('Commands/stubs/EloquentServiceProvider.stub'), $runtimeProviderContent);
                    File::copy(package_path('Commands/stubs/EloquentServiceProvider.stub'), app_path('/Providers/EloquentServiceProvider.php'));
                    File::put(package_path('Commands/stubs/EloquentServiceProvider.stub'), $defaultProviderContent);
        ...
        $defaultFacadeContent = File::get(package_path('Commands/stubs/DummyFacade.stub'));
        $runtimeFacadeContent = str_replace('DummyFacade', $name, $defaultFacadeContent);
        File::put(package_path('Commands/stubs/DummyFacade.stub'), $runtimeFacadeContent);
        File::copy(package_path('Commands/stubs/DummyFacade.stub'), app_path('/Facades/'.$name.'.php'));
        File::put(package_path('Commands/stubs/DummyFacade.stub'), $defaultFacadeContent);
